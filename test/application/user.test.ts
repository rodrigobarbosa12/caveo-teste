...
describe('editAccount', () => {
  ...
  it('deve editar um usuário com sucesso para o role admin', async () => {
    ...
    mockUserRepository.save.mockResolvedValue({ ...user, ...updatedData, isOnboarded: true });
  ...
  it('deve editar um usuário com sucesso para o role comum', async () => {
    ...
    mockUserRepository.findOneBy.mockResolvedValue(user);
    mockUserRepository.save.mockResolvedValue({ ...user, name: 'New Name', isOnboarded: true });
  ...
  it('deve lançar um erro se o usuário não for encontrado', async () => {
    ...
    await expect(editAccount(99, { name: 'Test', role: 'admin' } as User, 'admin'))
      .rejects
  ...
  it('deve lançar um erro se o usuário não tiver permissão', async () => {
    ...
    await expect(editAccount(3, { name: 'Test', role: 'admin' } as User, 'guest'))
      .rejects
...
describe('getOneUserByEmail', () => {
  ...
  it('deve retornar o usuário encontrado sem a senha', async () => {
    ...
    expect(result).toEqual({ id: 1, email: 'user@example.com', name: 'User 1', role: 'admin' });
